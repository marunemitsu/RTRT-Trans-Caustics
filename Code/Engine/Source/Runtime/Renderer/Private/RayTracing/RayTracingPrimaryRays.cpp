...
class FRayTracingPrimaryRaysRGS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		///SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, TransparencyOutput)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, CausticsColorOutput)
...
void FDeferredShadingSceneRenderer::RenderRayTracingPrimaryRaysView(
	...
	///FRDGTextureRef* InOutTransparencyColorTexture,
	FRDGTextureRef* InOutCausticsColorTexture,
	...
	ERayTracingPrimaryRaysFlag Flags)
{
...

	{
		...
		{
			*InOutColorTexture = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingTranslucent"));
		}

		/*if (*InOutTransparencyColorTexture == nullptr)
		{
			*InOutTransparencyColorTexture = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingPrimaryRays"));
		}*/

		if (*InOutCausticsColorTexture == nullptr)
		{
			*InOutCausticsColorTexture = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingCaustics"));
		}
		...
		{
			*InOutRayImaginaryDepthTexture = GraphBuilder.CreateTexture(Desc, TEXT("RayTracingPrimaryRaysImginaryDepth"));
		}
	}
	...
	PassParameters->CausticsColorOutput = GraphBuilder.CreateUAV(*InOutCausticsColorTexture);
	...
	///PassParameters->TransparencyOutput = GraphBuilder.CreateUAV(*InOutTransparencyColorTexture);
