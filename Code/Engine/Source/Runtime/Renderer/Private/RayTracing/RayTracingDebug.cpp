...
void FDeferredShadingSceneRenderer::RenderRayTracingDebug(FRHICommandListImmediate& RHICmdList, const FViewInfo& View)
{
	...
	{
		FRDGTextureRef BaseOutputColor = nullptr;
		FRDGTextureRef HitDistanceTexture = nullptr;
		FRDGTextureRef AdditiveTexture = nullptr;
		FRDGTextureRef BaseTexture = nullptr;
		FRDGTextureRef ImaginaryDepthTexture = nullptr;

		auto SceneColor = GraphBuilder.RegisterExternalTexture(SceneContext.GetSceneColor());

		RenderRayTracingPrimaryRaysView(
			GraphBuilder, View, &BaseOutputColor, &HitDistanceTexture, &AdditiveTexture, &ImaginaryDepthTexture,
			1, 1, 1,
			ERayTracingPrimaryRaysFlag::ConsiderSurfaceScatter);

		AddDrawTexturePass(GraphBuilder, View, BaseOutputColor, SceneColor, View.ViewRect.Min, View.ViewRect.Min, View.ViewRect.Size());
